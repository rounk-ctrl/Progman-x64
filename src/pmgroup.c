	...
	LPTSTR lpText)
{
	...
	if (pt.x == -1) {
		...
		dwStyle = GetWindowLongPtr(pGroup->hwnd,GWL_STYLE);
...
LRESULT APIENTRY GroupWndProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_RBUTTONUP:
	{
		...
		ClientToScreen(hwnd, &pt); // needed or else context menu goes to the top of the screen
		SetRect(&rcDrag, 0, 0, 0, 0);

		pItem = ItemHitTest(GetWindowLongPtr(hwnd, GWLP_PGROUP), MAKEPOINTS(lParam));

		if (!pItem) {
			ProgmanCommandProc(hwnd, ActivateCommonContextMenu(hwnd, IDCM_GROUP, hwnd, MAKELPARAM(pt.x, pt.y)), lParam);
			break;
		}
		else {
			ProgmanCommandProc(hwnd, ActivateCommonContextMenu(hwnd, IDCM_ITEM, hwnd, MAKELPARAM(pt.x, pt.y)), lParam);
			break;
		}
		break;
	}
	...
	case WM_MOUSEMOVE:
		if (wParam & MK_LBUTTON) {
			pt.x = GET_X_LPARAM(lParam); // (int)(MAKEPOINTS(lParam).x);
			pt.y = GET_Y_LPARAM(lParam); // (int)(MAKEPOINTS(lParam).y);
	...
	case WM_SYSCOMMAND:
	{
		...
		if (wParam == SC_MAXIMIZE)
			 SetWindowLongPtr(hwnd, GWL_STYLE,
				  (GetWindowLongPtr(hwnd,GWL_STYLE) & ~(WS_HSCROLL | WS_VSCROLL)));
		  goto DefProc;
...
// Context menu!
BOOL
ActivateCommonContextMenu(HWND hwnd, UINT uiHitType, WPARAM wParam, LPARAM lParam)
{
	HMENU hMenu = NULL;
	BOOL bPopup = NULL;
	...
	switch (uiHitType) {
	case (IDCM_ITEM):
		hMenu = GetSubMenu(LoadMenu(hAppInstance, TEXT("CTXMENU_ITEM")), 0);
		break;

	case (IDCM_GROUP):
		hMenu = GetSubMenu(LoadMenu(hAppInstance, TEXT("CTXMENU_GROUP")), 0);
		break;
	...
	bPopup = TrackPopupMenu(hMenu, TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RETURNCMD, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam), 0, (HWND)wParam, NULL);
