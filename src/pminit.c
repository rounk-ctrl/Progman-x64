...
HWND APIENTRY CreateFrameWindow(register PRECT prc, WORD nCmdShow)
{
	...
	// Add Run to the window controls menu
	LoadString(hAppInstance, IDS_RUN, szBuffer, CharSizeOf(szBuffer));
	InsertMenu(hSystemMenu, 6, MF_BYPOSITION | MF_STRING, IDM_RUN, szBuffer);
...
BOOL APIENTRY AppInit(HANDLE hInstance, LPTSTR lpszCmdLine, int nCmdShow)
{
	...
	// 2 means Progman will shutdown before taskman (level = 1) and
	...
	if (hwndPrev = FindWindow(szClass, NULL)) {
		...
		if (IsHandleReallyProgman(hwndPrev)) {
			...
			// If so, then we will continue.  GetWindowLongPtr returns 0 as a
			// failure case and as the "Not exiting" case (1 if we are exiting),
			// so we need to confirm that the last error is also zero.
			//

			lExiting = GetWindowLongPtr(hwndPrev, GWL_EXITING);
...
//  MarkProcess(MP_SYSTEMAPP);
   ...
   */
	...
	if (hhkMsgFilter == 0) {
		GetLastError(); // BUG: return not used?
	}
	/*
   ...
   */
	...
	/*wndClass.hInstance        = hAppInstance;*/
	wndClass.hIcon            = hGroupIcon = LoadIcon(hAppInstance, (LPTSTR)MAKEINTRESOURCE(GROUPICON));
	/*wndClass.hCursor          = LoadCursor(NULL, IDC_ARROW);*/
