	...
	HWND hwndLB)
{
   ...
   DWORD lTemp;
	 ...
	 */
	lTemp = GetWindowLongPtr(hwndLastActive, GWL_STYLE);
...
INT_PTR APIENTRY TaskmanDlgProc(
	HWND hwnd,
	UINT wMsg,
	WPARAM wParam,
	LPARAM lparam)
{
	...
	case WM_INITDIALOG:
		...
		SendDlgItemMessage(hwnd, IDD_TMPATH, EM_LIMITTEXT, MAX_PATH -4, 0L);
	...
	case MYCBN_SELCHANGE:
	   if (!fNoRun) {
		  if (GetDlgItemText(hwnd, IDD_TMPATH, (LPTSTR)szTMPathField, MAX_PATH)) {
	...
	case WM_COMMAND:
		...
		case IDD_RUN:
		   if (!fNoRun) {
			  ...
			  GetDlgItemText(hwnd, IDD_TMPATH, szTMPathField, MAX_PATH);
			  DoEnvironmentSubst(szTMPathField, MAX_PATH);
...
InitTaskman()
{
   ...
   GetCurrentDirectory(MAX_PATH, szTMUserHomeDir);
   GetWindowsDirectory(szTMWindowsDirectory, MAX_PATH);
   ...
   // This is where CTRL + ESC is registered, removed "RegisterTaskList"
   if (!RegisterHotKey(ghwndTMDialog, HOTKEY_TASKMAN, MOD_CONTROL, VK_ESCAPE)) {
