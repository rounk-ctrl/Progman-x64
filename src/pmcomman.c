	...
	)
{
  ...
  /*changed order, since wPendINstance is a 32b HANDLE, and ret is WORD*/
	...
	ret = ShellExecute(NULL, NULL, lpszPath, lpP, lpDir, (WORD)(bLoadIt ? SW_SHOWMINNOACTIVE : SW_SHOWNORMAL));
...
VOID APIENTRY ExecItem(PGROUP pGroup, PITEM pItem, BOOL fShift, BOOL fStartup)
{
	...
	pExecingItem = pItem;
	DoEnvironmentSubst(szCommand, MAX_PATH);
	DoEnvironmentSubst(szDir, 2 * (MAX_PATH));
...
VOID RemoveAnsiGroups()
{
	...
	if (!RegDeleteKey(HKEY_CURRENT_USER, szAnsiProgramGroups)) {
		...
		//
		if (pCurrentGroup &&
			  GetWindowLongPtr(pCurrentGroup->hwnd, GWL_STYLE) & WS_MAXIMIZE) {
...
BOOL APIENTRY ProgmanCommandProc(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
	...
	if (hwndMdiActive != NULL)
		bMaxed = (GetWindowLongPtr(hwndMdiActive, GWL_STYLE) & WS_MAXIMIZE) ? TRUE : FALSE;
	else
		bMaxed = 0;
...
PCPNewItem:
	...
	case IDM_SHUTDOWN:
		...
		if (bExitWindows) {
			...
			fExiting = TRUE;
			SetWindowLongPtr(hwndProgman, GWL_EXITING, 1);

			/* Call the ShutdownDlg API. */
			ShutdownDlg(hwndProgman);
...
ACPCallHelp:
	  ...
	  case IDM_ABOUT:
	  {
		  ...
		  ShellAbout(hwndProgman, szTitle, NULL, hIcon);
		  break;
	  }
